...
public class TileLayerLoader extends LayerLoader {
     ...
     */
    @Override
    public TileLayer load(XmlNode node) throws IOException {
    ...
    private void readData(TileLayer layer, XmlNode node) throws IOException {
    ...
    private void decodeBase64Data(TileContainer tileContainer, XmlNode node, DataCompression compression) throws IOException {
    ...
    private void decodeCsvData(TileContainer tileContainer, XmlNode node) throws IOException {
    ...
    private void decodeTileData(TileContainer tileContainer, XmlNode node) {
        int x = 0;
        int y = 0;
        int width = tileContainer.getWidth();
        int height = tileContainer.getHeight();
     ...
     * @return the chunk
     */
    private Chunk readChunk(TileLayer layer, XmlNode node, DataEncoding encoding, DataCompression compression) throws IOException {
    ...
    private void readTileProperties(TileLayer layer, XmlNode node) {
        XmlNode child = node.getFirstChild();
        while (child != null) {
