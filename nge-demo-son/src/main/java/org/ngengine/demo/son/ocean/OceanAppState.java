...
public class OceanAppState
    ...
    public void onEnable(ComponentManager fragmentManager, Runner runner, DataStoreProvider dataStoreProvider,
        ...
        if(ibocean==null){
            ibocean = OceanLayerGenerator.generateOcean(
                ...
                sxx.depthFirstTraversal(sx->{
                    if(sx instanceof Geometry) {
                        Geometry geom = (Geometry)sx;
                            ...
                            mat.getAdditionalRenderState().setStencil(true, 
    ...
    private void updateReflectionCam(Camera sceneCam, Camera reflectionCam, Plane plane) {
        ...
        try {
            ...
            if (isShallowAngle) {
                ...
                float shallowAngleBoost = (angleToWater - (FastMath.HALF_PI * 0.7f))
                        / (FastMath.HALF_PI * 0.3f);
            ...
            reflectionCam.setFrustumPerspective(fixedFOV, aspect, 0.1f, // Smaller near plane helps with close
                                                                        // reflections
