...
plugin generates metadata. By default, those classes are also emitted with
`unsafeAllocated: true` metadata. Opt out when a type must only be available for
reflection:

```java
@PreserveReflection(allowUnsafeAllocation = false)
public class MyReflectiveOnlyType {
}
```
...
Manual unsafe allocation entries can be added through the DSL:
